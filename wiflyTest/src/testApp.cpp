...
void testApp::update(){
	...
	if(message!=""){
		...
		for(unsigned int i=0;i<strPoints.size();i++){
			vector<string> point = ofSplitString(strPoints[i],"/t");
			if( point.size() == 6 ){
				aX=atof(point[0].c_str());
				aY=atof(point[1].c_str());
                aZ=atof(point[2].c_str());
                cout << "Accel " << aX << ", " << aY << ", " << aZ << endl;
                accel.push_back(ofPoint(aX,aY,aZ));
                mX=atof(point[3].c_str());
				mY=atof(point[4].c_str());
                mZ=atof(point[5].c_str());
